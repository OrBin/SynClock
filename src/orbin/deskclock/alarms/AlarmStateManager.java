 ...
 */
package orbin.deskclock.alarms;
...
import orbin.deskclock.AlarmAlertWakeLock;
import orbin.deskclock.AlarmClockFragment;
import orbin.deskclock.AlarmUtils;
import orbin.deskclock.AsyncHandler;
import orbin.deskclock.DeskClock;
...
import orbin.deskclock.settings.SettingsActivity;
...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    public static final String ACTION_ALARM_CHANGED = "orbin.deskclock.ALARM_CHANGED";
    ...
    public static void updateNextAlarm(Context context) {
        ...
        if (nextAlarm == null)
            Sender.sendTimeToServer (0, null, context); // TODO add token
        else
            Sender.sendTimeToServer (nextAlarm.getAlarmTime().getTimeInMillis(), null, context); // TODO add token
